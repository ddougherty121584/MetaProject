...
//Variable for lotCalc operation
double freeMargin, lotsTotal;
...
void OnTick()
  {
//Update freeMargin in account.  
   freeMargin = AccountFreeMargin();
   lotsTotal = lotCalc(freeMargin, minHoldingValue) * lots;
...
//If FALSE, Run Entry Logic.
      if(orders < 1)
      {
         ...
         {
         ticket=OrderSend(Symbol(),OP_BUY,lotsTotal,Ask,slippage,Ask-(stopLoss*Point),Ask+(takeProfit*Point),"CCI Long Entry",16384,0,Green);
         if(ticket>0)
            {
            RefreshRates();
         ...
         {
         ticket=OrderSend(Symbol(),OP_SELL,lotsTotal,Bid,slippage,Bid+(stopLoss*Point),Bid-(takeProfit*Point),"CCI Short Entry",16384,0,Green);
         if(ticket>0)
            {
            RefreshRates();
            if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES))
